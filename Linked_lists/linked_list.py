...
def merge_sorted_lists(l1,l2):
    ...
    while not (fin1 and fin2):
        ...
        if not fin1 and (fin2 or node1.val<node2.val):
            ll.insert(node1.val)
            try:
                node1=next(it1)
        ...
        #if not fin2 and (fin1 or node2.val<node1.val):
            ll.insert(node2.val)
            try:
                node2=next(it2)
            except StopIteration:
...
def merge_sorted_lists_inplace(l1,l2):
    ...
    node1=l1.beg.next
    node2=l2.beg.next
    root=node=node1 if node1.val<node2.val else node2
    while node1.val or node2.val:
        #print(node.val,node1.val,node2.val)
        if node1.val and (not node2.val or node1.val<node2.val):
            node.next,node1=node1,node1.next

        else:
            
            node.next,node2=node2,node2.next
        node,node.next.prev=node.next,node
...
l1=LL()
l2=LL()
l2.multi_insert(1,2,3,4,5,20,21,22)
l1.multi_insert(11,12,13)

l_merged=merge_sorted_lists(l1,l2)
l_merged.print_all()
l_merged.reverse(0,10)
l_merged.reverse(0,10)
l_merged.print_all(rev=False)
l_merged.cyclic_shift(2)
l_merged.print_all(rev=False)
#l_merged.cyclic_shift(1)
l_merged.print_all(rev=False)
print(l_merged.get(1,l_merged.beg).val)
print(l1[-3].val)

root=merge_sorted_lists_inplace(l1,l2)
